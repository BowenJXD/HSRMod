...
public class ChangeSceneEffect extends AbstractGameEffect {
    ...
    public ChangeSceneEffect(Texture img) {
        ...
        this.img = img;
        this.img2 = ImageMaster.loadImage("HSRModResources/img/effects/Territory/Mask.png");
    ...
    public void update() {
        if (this.right) {
            if (this.x < 0.0F) {
                this.x += 3000.0F * Gdx.graphics.getDeltaTime() * Settings.scale;
    ...
    public void render(SpriteBatch sb) {
        ...
        if (this.x == 0.0F) {
            sb.draw(this.img, 0.0F, 0.0F, (float) Settings.WIDTH, (float) Settings.WIDTH / (float) this.img.getWidth() * 1.0F * (float) this.img.getHeight());
        } else {
            this.drawAlphaMask(sb);
            this.drawForeground(sb, (int) this.x, 0, (int) ((float) Settings.WIDTH / 0.8F), Settings.HEIGHT);
        }
    ...
    private void drawAlphaMask(SpriteBatch sb) {
        ...
        if (this.right) {
            sb.draw(this.img2, this.x, 0.0F, (float) Settings.WIDTH / 0.8F, (float) Settings.HEIGHT);
        } else {
            sb.draw(this.img3, this.x, 0.0F, (float) Settings.WIDTH / 0.8F, (float) Settings.HEIGHT);
        }
