...
public abstract class AbstractCard implements Comparable<AbstractCard> {
        ...
        if (Settings.lineBreakViaCharacter) {
            ...
            for(String word : this.rawDescription.split(" ")) {
                ...
                if (GameDictionary.keywords.containsKey(keywordTmp)) {
                    ...
                    GlyphLayout var10000 = gl;
                    var10000.width += tmp;
                    isKeyword = true;
                } else if (!word.isEmpty() && word.charAt(0) == '[') {
                    gl.reset();
                    gl.setText(FontHelper.cardDescFont_N, sbuilder2);
                    GlyphLayout var13 = gl;
                    var13.width += CARD_ENERGY_IMG_WIDTH;
    ...
    public void initializeDescriptionCN() {
        ...
        for(int var12 = 0; var12 < this.description.size(); ++var12) {
            if (((DescriptionLine)this.description.get(var12)).text.equals(LocalizedStrings.PERIOD)) {
                StringBuilder var10000 = new StringBuilder();
                DescriptionLine var10002 = (DescriptionLine)this.description.get(var12 - 1);
    ...
    private void renderDescription(SpriteBatch sb) {
        if (this.isSeen && !this.isLocked) {
            ...
            for(int i = 0; i < this.description.size(); ++i) {
                ...
                for(String tmp : ((DescriptionLine)this.description.get(i)).getCachedTokenizedText()) {
        ...
        if (this.isSeen && !this.isLocked) {
            ...
            for(int i = 0; i < this.description.size(); ++i) {
                ...
                for(String tmp : ((DescriptionLine)this.description.get(i)).getCachedTokenizedTextCN()) {
                    ...
                    for(int var23 = 0; var23 < tmp.length(); ++var23) {
                        if (tmp.charAt(var23) == 'D' || tmp.charAt(var23) == 'B' && !tmp.contains("[B]") || tmp.charAt(var23) == 'M') {
                            updateTmp = tmp.substring(0, var23);
                            updateTmp = updateTmp + this.getDynamicValue(tmp.charAt(var23));
                            updateTmp = updateTmp + tmp.substring(var23 + 1);
                            break;
                        }
                    }
    ...
    public void applyPowers() {
        ...
        if (!this.isMultiDamage) {
            ...
            for(int var16 = 0; var16 < tmp.length; ++var16) {
                for(AbstractRelic r : player.relics) {
                    tmp[var16] = r.atDamageModify(tmp[var16], this);
                    if (this.baseDamage != (int)tmp[var16]) {
                        this.isDamageModified = true;
                    }
                }

                for(AbstractPower p : player.powers) {
                    tmp[var16] = p.atDamageGive(tmp[var16], this.damageTypeForTurn, this);
                }

                tmp[var16] = player.stance.atDamageGive(tmp[var16], this.damageTypeForTurn, this);
                if (this.baseDamage != (int)tmp[var16]) {
                    this.isDamageModified = true;
                }
            }

            for(int var17 = 0; var17 < tmp.length; ++var17) {
                for(AbstractPower p : player.powers) {
                    tmp[var17] = p.atDamageFinalGive(tmp[var17], this.damageTypeForTurn, this);
                }
            }

            for(int var18 = 0; var18 < tmp.length; ++var18) {
                if (tmp[var18] < 0.0F) {
                    tmp[var18] = 0.0F;
                }
            }
            ...
            for(int var19 = 0; var19 < tmp.length; ++var19) {
                if (this.baseDamage != (int)tmp[var19]) {
                    this.isDamageModified = true;
                }

                this.multiDamage[var19] = MathUtils.floor(tmp[var19]);
            }
    ...
    public void calculateCardDamage(AbstractMonster mo) {
        ...
        if (!this.isMultiDamage && mo != null) {
            ...
            for(int var17 = 0; var17 < tmp.length; ++var17) {
                for(AbstractPower p : ((AbstractMonster)m.get(var17)).powers) {
                    if (!((AbstractMonster)m.get(var17)).isDying && !((AbstractMonster)m.get(var17)).isEscaping) {
                        tmp[var17] = p.atDamageReceive(tmp[var17], this.damageTypeForTurn, this);
                    }
                }
            }
            ...
            for(int var18 = 0; var18 < tmp.length; ++var18) {
                for(AbstractPower p : player.powers) {
                    tmp[var18] = p.atDamageFinalGive(tmp[var18], this.damageTypeForTurn, this);
                }
            }

            for(int var19 = 0; var19 < tmp.length; ++var19) {
                for(AbstractPower p : ((AbstractMonster)m.get(var19)).powers) {
                    if (!((AbstractMonster)m.get(var19)).isDying && !((AbstractMonster)m.get(var19)).isEscaping) {
                        tmp[var19] = p.atDamageFinalReceive(tmp[var19], this.damageTypeForTurn, this);
                    }
                }
            }

            for(int var20 = 0; var20 < tmp.length; ++var20) {
                if (tmp[var20] < 0.0F) {
                    tmp[var20] = 0.0F;
                }
            }
            ...
            for(int var21 = 0; var21 < tmp.length; ++var21) {
                if (this.baseDamage != MathUtils.floor(tmp[var21])) {
                    this.isDamageModified = true;
                }

                this.multiDamage[var21] = MathUtils.floor(tmp[var21]);
            }
