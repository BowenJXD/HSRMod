...
public class Vagrant extends BaseMonster {
    public static final String ID = Vagrant.class.getSimpleName();

    int block = 16;
    int goldAmt = 15;
    int stolenGold = 0;

    public Vagrant(float x, float y) {
        super(ID, 200, 256, x, y);
        setDamagesWithAscension(8);
        block = ModHelper.moreHPAscension(type) ? 18 : 16;
        ...
        switch (nextMove) {
            case 0:
                ...
                AbstractDungeon.actionManager.addToBottom(new AbstractGameAction() {
                    public void update() {
                        Vagrant.this.stolenGold = Vagrant.this.stolenGold + Math.min(Vagrant.this.goldAmt, AbstractDungeon.player.gold);
                        this.isDone = true;
                    }
                });
